...
class SmilesIterator(Iterator):
    ...
    def next(self):
        ...
        batch_x = np.zeros(tuple([current_batch_size] + [self.smiles_data_generator.pad,
                                                         self.smiles_data_generator.charlen]), dtype=self.dtype)
...
class SmilesEnumerator(object):
    ...
    """
    def __init__(self, charset='@C)(=cOn1S2/H[N]\\', pad=120, leftpad=True, isomeric_smiles=True, enum=True,
                 canonical=False):
        self._charset = None
    ...
    def transform(self, smiles):
        ...
        #Arguments
            smiles: Numpy array or Pandas series containing smiles as strings
        """
        one_hot = np.zeros((smiles.shape[0], self.pad, self._char_len), dtype=np.int8)
...
if __name__ == "__main__":
    ...
    # BUG, when batch size > x.shape[0], then it only returns x.shape[0]!
    ...
    if len(X) != 10:
        print("Error in batch size generation")
